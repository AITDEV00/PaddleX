...
> (for wiring gaps), **external contract verification** (for protocol drift), and
> a fix-then-recheck loop (for cascading side effects).
...
| **Protocol drift — our schema diverges from upstream** | Our `error_msg` vs docling's `error_message`; `component_type: str` vs `DoclingComponentType` enum | **L3: External contract** |
| **Non-convergence — reviewer brings new external knowledge each pass** | Pass 1: knows `logger.exception` is better. Pass 2: thinks to check docling's `ErrorItem`. Each pass finds a *different* class of bug. | **L3: External contract (systematizes the check so it's not ad-hoc)** |
...
### Schema ↔ External Contract Cross-Check

```python
# Given our response model:
class ErrorItem(BaseModel):
    component_type: str = "ocr"       # ← does docling use str or an enum?
    error_type: str                    # ← does docling even have this field?
    error_msg: str                     # ← or is it "error_message"?

# Given docling's actual ErrorItem (from docling.datamodel.base_models):
class ErrorItem(BaseModel):
...
**Technique**: For every model class whose docstring says "mirrors" or
"mirrors docling-serve" or "matches protocol X", retrieve the **upstream
definition** and diff the field lists:

1. **List our fields** — `list(Model.model_fields.keys())`
2. **List upstream fields** — from the installed package, the GitHub repo,
   or the API spec
3. **Diff the two lists** — any field name mismatch is a compatibility bug

Check **all three dimensions** of each field:
- **Name**: `error_msg` vs `error_message` — clients parse by name
- **Type**: `str` vs `DoclingComponentType` — enum values must match
- **Default**: `"ocr"` vs `DoclingComponentType.MODEL` — invalid enum value

**Why this is outside L1-L3**: L3 cross-references check our code against
*itself* (enum members ↔ dispatch tables, constants ↔ usages). It cannot
detect that our `error_msg` field should be `error_message` because both
names are internally consistent — the field is defined, populated, and
serialized correctly *by our own code*. The bug only surfaces when a client
expecting the docling protocol reads `error_msg` and can't find `error_message`.

**Where to find upstream definitions**:

| Source | When to use | How |
|--------|------------|-----|
| Installed pip package | Package is in the container | `python3 -c "from X import Y; print(list(Y.model_fields.keys()))"` |
| GitHub repo | Package not installed | `gh search code "class ErrorItem" --repo owner/repo` |
| API spec / OpenAPI | Protocol defined by spec | Fetch the spec, parse schema definitions |
| Test fixtures | Both above unavailable | Search repo test files for model construction |

**Automation**:
```bash
# List all "mirrors" claims in docstrings
grep -rn "mirror\|mirrors\|matches.*protocol\|compatible with" $API_DIR --include="*.py"

# For each hit, retrieve the upstream definition and diff field lists
# (manual or scripted — the point is to do it EVERY pass, not ad-hoc)
```

**Rule**: Any model with a "mirrors" docstring claim MUST have its field
list diffed against the upstream source every audit pass. This is not
optional — it is the single most common source of non-convergence.
